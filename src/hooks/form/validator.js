      ...
      for (const validKey in data[key].validate) {
        switch (validKey) {
          ...
          case "max":
            if (
              data[key].val &&
              data[key].val.length > data[key].validate.max
            ) {
              data[key].isValid = false;
              data[key].validate.message =
                "بیشتر از" +
          ...
          case "min":
            if (
              data[key].val &&
              data[key].val.length < data[key].validate.min
            ) {
              data[key].isValid = false;
              data[key].validate.message =
                "کمتر از" +
          ...
          case "equal":
            if (
              data[key].val &&
              data[key].val.length !== data[key].validate.equal
            ) {
              console.log("is not euqal");
              data[key].isValid = false;
              data[key].validate.message =
                "باید" + " " + data[key].validate.equal + " " + "کاراکتر باشد";
              formIsValid = false;
            }
